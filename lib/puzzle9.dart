...
class _HomeState extends State<Home> {
  ...
  bool _isSolvable(List<_BoxItem?> tiles) {
    List<_BoxItem> withoutNull = tiles.whereType<_BoxItem>().toList();
    int inversions = 0;
        ...
        children: [
          Align(
            ...
            child: SizedBox(
              width: tileSize * 3,
              height: tileSize * 3,
              child: _buildAnimatedGrid(current, size: tileSize),
            ),
          ),
          Align(
  ...
  Widget _buildAnimatedGrid(List<_BoxItem?> items, {required double size}) {
    return Stack(
      children: List.generate(9, (i) {
        final item = items[i];
        if (item == null) return const SizedBox.shrink();

        final index = items.indexOf(item); // Actual posición lógica
        final row = index ~/ 3;
        final col = index % 3;
  ...
  void _confirmReset() {
    showDialog(
      ...
      builder:
          (context) => AlertDialog(
            title: const Text('¿Reiniciar puzzle?'),
            content: const Text(
              '¿Estás seguro de que deseas reiniciar el puzzle actual?',
            ),
            actions: [
              TextButton(
              ...
              TextButton(
                onPressed: () {
                  ...
                  _shuffleTiles(); // Reiniciar puzzle
                },
